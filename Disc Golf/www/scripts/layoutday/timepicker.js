...
// 分钟
var minuteStr = new Array( 4 ).fill( null ).map(function(t,i){
	var val = twobit( i * 15 ); 
...
var content = $('<div class="timepicker">\
		<div v-show class="title">Pick a Time</div>\
            ...
            <center>\
                <a id="" class="js-ok-time ui-shadow ui-btn ui-corner-all" onclick="" style="font-size:70%; width: 75%">OK</a>\
            </center>\
		</div>\
	</div>');
...
timepicker.bindEvent = function () {
	...
	this.content.on('click','.js-minus-minute',function() {
		...
		if( minute <= 0 ){
			thisTimePicker.minute = 45;
		} else {
			thisTimePicker.minute -= 15;
		}
