(function($) {
	var
		...
		picker_render_calendar = function() {
            ...
            for (var i = 1; i <= _NUM(_S); i++) {
                ...
                //If we have a meetup on this day then colour it 
                //If we contain the contains-meetup
                var meetupDate = i + "/" + get_current('m') + "/" + get_current('y');
                
                if (meetupsArray.hasOwnProperty(meetupDate)) {
                    w.find('li').eq(index)
                        .html(i)
                        .addClass('pick-n pick-v contains-meetup') //ADDED THIS CLASS ON MYSELF
                        .attr('data-value', i);
                }
                else {
                    w.find('li').eq(index)
	...
	.on(ui_event.i,picker_node_el+' .pick-submit',function(){
        picker_hide();
        ShowMeetups(document.getElementById('meetupDay').value);                                      //ADDED TO REFRESH CALENDAR
	});
